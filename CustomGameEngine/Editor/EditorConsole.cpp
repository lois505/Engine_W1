...
EditorConsole::EditorConsole()
	: HistoryPos(-1), AutoScroll(true), ScrollToBottom(false)
{
	Logger::Bind([this](const char* message) { AddLog("%s", message); });
...
{
	AddLog("# %s\n", command_line);
	// Insert into history. First find match and delete it so it always goes to the end.
	HistoryPos = -1;
...
int EditorConsole::TextEditCallbackStub(ImGuiInputTextCallbackData* data)
{
