...
namespace json {
    ...
    JSON& JSON::at(const string& key) { return operator[](key); }
    const JSON& JSON::at(const string& key) const { return Internal.Map->at(key); }
    JSON& JSON::at(unsigned index) { return operator[](index); }
    const JSON& JSON::at(unsigned index) const { return Internal.List->at(index); }

    int JSON::length() const {
        return (Type == Class::Array) ? (int)Internal.List->size() : -1;
    }
    ...
    int JSON::size() const {
        if (Type == Class::Object) return (int)Internal.Map->size();
        if (Type == Class::Array) return (int)Internal.List->size();
        return -1;
    }
    ...
    string JSON::dump(int depth, string tab) const {
        ...
        case Class::Boolean:  return Internal.Bool ? "true" : "false";
        default: return "";
        }
    }
    ...
    namespace {
        ...
        void consume_ws(const string& str, size_t& offset) {
            while (offset < str.size() && isspace(str[offset])) ++offset;
        }
        ...
        JSON parse_object(const string& str, size_t& offset) {
            ...
            if (str[offset] == '}') { ++offset; return std::move(Object); }
            ...
            while (true) {
                ...
                else if (str[offset] == '}') { ++offset; break; }
                else {
        ...
        JSON parse_array(const string& str, size_t& offset) {
            ...
            ++offset;
            consume_ws(str, offset);
            if (str[offset] == ']') { ++offset; return std::move(Array); }
                    ...
                    case 'u': {
                        ...
                        for (unsigned i = 1; i <= 4; ++i) {
                            ...
                            if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'))
                                ...
                                std::cerr << "ERROR: String: Expected hex character, found '" << c << "'\n";
            ...
            if (c == 'E' || c == 'e') {
                ...
                while (true) {
                    ...
                    if (c >= '0' && c <= '9') exp_str += c;
                        ...
                        std::cerr << "ERROR: Number: Expected exponent, found '" << c << "'\n";
                        return std::move(JSON::Make(JSON::Class::Null));
                    }
                    else break;
                }
                if (!exp_str.empty()) exp = std::stol(exp_str);
            }
            else if (!isspace(c) && c != ',' && c != ']' && c != '}') {
                // Handled later by --offset
            }
            --offset;

            if (isDouble) Number = std::stod(val) * std::pow(10, exp);
            else {
                if (!exp_str.empty()) Number = std::stol(val) * std::pow(10, exp);
                else Number = std::stol(val);
            }
        ...
        JSON parse_bool(const string& str, size_t& offset) {
            JSON Bool;
            if (str.substr(offset, 4) == "true") { Bool = true; offset += 4; }
            else if (str.substr(offset, 5) == "false") { Bool = false; offset += 5; }
            else {
        ...
        JSON parse_next(const string& str, size_t& offset) {
            ...
            std::cerr << "ERROR: Parse: Unknown character '" << value << "'\n";
            return JSON();
        }
    }
